...
public struct SwiftMD5{
    public func F(_ b: UInt32, _ c: UInt32, _ d: UInt32) -> UInt32 {
        return (b & c) | ((~b) & d)
    }

    public func G(_ b: UInt32, _ c: UInt32, _ d: UInt32) -> UInt32 {
        return (b & d) | (c & (~d))
    }

    public func H(_ b: UInt32, _ c: UInt32, _ d: UInt32) -> UInt32 {
        return b ^ c ^ d
    }

    public func I(_ b: UInt32, _ c: UInt32, _ d: UInt32) -> UInt32 {
        return c ^ (b | (~d))
    }

    public func rotateLeft(_ x: UInt32, by: UInt32) -> UInt32 {
        return ((x << by) & 0xFFFFFFFF) | (x >> (32 - by))
    }

    // MARK: - Calculating a MD5 digest of bytes from bytes
    public func md5(_ bytes: [UInt8]) -> Digest {
        // Initialization
        let s: [UInt32] = [
